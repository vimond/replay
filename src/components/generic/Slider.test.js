...
function shallowRenderSlider({ value=10, maxValue=45, classNamePrefix, className, handleClassName='myitemclassname', trackContent, children, handleContent, label='My slider', onValueChange, onDrag } = {}) {
    return shallow(<Slider
        className={className}
...
test('The slider handle should be positioned to the left/bottom when specifying values or max values not making sense or not being numbers.',() => {
    ...
    expect(shallowRenderSlider({ value: 0, maxValue: 0, handleContent: mockHandleContent }).childAt(1).prop('style')).toHaveProperty('left', '0%');
    expect(shallowRenderSlider({ value: 0, maxValue: NaN, handleContent: mockHandleContent }).childAt(1).prop('style')).toHaveProperty('left', '0%');
    expect(shallowRenderSlider({ value: 0, maxValue: null, handleContent: mockHandleContent }).childAt(1).prop('style')).toHaveProperty('left', '0%');
    expect(shallowRenderSlider({ value: { yes: 'no' }, maxValue: 167671, handleContent: mockHandleContent }).childAt(1).prop('style')).toHaveProperty('left', '0%');
...
test('When clicking on a slider position, the slider should report the updated absolute value between 0 and the specified max value.', () => {
    ...
    const shallowElement = shallowRenderSlider({ value: 20, maxValue: 400, handleContent: mockHandleContent, onValueChange: handleValueChange });
...
test('The slider should not update the UI based on new props while the handle is pressed. However it needs fall back to the last set value in props when the handle is released.', () => {
    ...
    const handleValueChange = jest.fn();
    const handleDrag = jest.fn();
    const shallowElement = shallowRenderSlider({ value: 20, maxValue: 400, handleContent: mockHandleContent, onValueChange: handleValueChange, onDrag: handleDrag });
    shallowElement.instance().renderedTrack = mockEventElement;
