...
test('<GotoLiveButton/> renders with prefixed class name and DOM including children.', () => {
	const rendered = shallow(<GotoLiveButton playMode="livedvr" classNamePrefix="a-" isAtLivePositionContent="A" isNotAtLivePositionContent="B" label="C"/>);
...
test('<GotoLiveButton/> does not render if stream is stream is on demand or has no DVR window.', () => {
	const rendered = shallow(<GotoLiveButton playMode="ondemand" classNamePrefix="a-" isAtLivePositionContent="A" isNotAtLivePositionContent="B" label="C"/>);
	expect(rendered.getElement()).toBe(null);
	const rendered2 = shallow(<GotoLiveButton playMode="live" classNamePrefix="a-" isAtLivePositionContent="A" isNotAtLivePositionContent="B" label="C"/>);
	expect(rendered2.getElement()).toBe(null);
});

test('<GotoLiveButton/> renders button state when live correctly.', () => {
	const rendered = shallow(<GotoLiveButton classNamePrefix="a-" playMode="livedvr" isAtLivePosition={true} isAtLivePositionContent="A" isNotAtLivePositionContent="B"/>);
	const renderedToggleButton = rendered.dive();
...
test('<GotoLiveButton/> invokes the gotoLive() callback when clicked, and not at live position.', () => {
	...
	const rendered = shallow(<GotoLiveButton gotoLive={gotoLiveCallback} playMode="livedvr" isAtLivePosition={false}/>);
...
test('<GotoLiveButton/> does not invoke the gotoLive() callback when clicked, if already at live position.', () => {
	const gotoLiveCallback = jest.fn();
	const rendered = shallow(<GotoLiveButton gotoLive={gotoLiveCallback} playMode="livedvr" isAtLivePosition={true}/>);
	expect(rendered.props().isOn).toBe(true);
	const renderedToggleButton = rendered.dive();
	expect(renderedToggleButton.simulate('click'));
