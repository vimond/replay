...
All class names applied to DOM elements of controls and container components are prefixed. The default prefix is `replay-`, and a full class name will then for instance be `replay-play-pause-button`. The prefix can be changed when creating custom players, or simply by including a configuration override (TODO: link). In this way, the player can get different skins coexisting in the same CSS scope. 
