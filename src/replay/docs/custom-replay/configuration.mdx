...
*Most of the player code examples in this guide use the [MockVideoStreamer](TODO: link), so that the live examples don't load actual videos.*
...
The object above specifies behaviours for controls, maps keyboard shortcuts, and enables storing the user's preferences in the brower's local storage. 
