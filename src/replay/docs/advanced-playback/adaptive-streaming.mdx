...
import { Replay } from 'vimond-replay';
import 'vimond-replay/index.css';
import CompoundVideoStreamer from 'vimond-replay/video-streamer/CompoundVideoStreamer/CompoundVideoStreamer';
```
...
The following example selects the ShakaVideoStreamer for the specified source string. The manifest URL contains `.mpd`, which makes the compound video streamer guess that this is a MPEG-DASH manifest.
