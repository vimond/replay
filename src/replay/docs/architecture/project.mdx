...
* `yarn prettier`. Will reformat code.
* `yarn flow`
* `yarn test`
* `yarn eslint src/replay` should display no warnings.

These steps will be improved and automated in the future. The three QA-related commands are also part of the automated build, which means pushing modifications will lead to failing builds if errors are not fixed before push.
