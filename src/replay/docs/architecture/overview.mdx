...
1. The video streamer's responsibility is to download and render the stream. It exposes updates to the stream playback state through callback notifications.
...
3. The controls can observe such state updates by being wrapped in a HOC connecting to the React context provided by the player controller.
