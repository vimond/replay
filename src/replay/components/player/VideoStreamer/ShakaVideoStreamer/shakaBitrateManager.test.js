...
test('Shaka bitrate manager capBitrate() with valid number passed restricts to a max bandwidth, but enables ABR.', () => {
  const { eventHandlers, onStreamStateChange, shakaPlayer, bitrateManager } = setup([1234567, 2345678, 7891234, 3456789]);
  eventHandlers.streaming();
  eventHandlers.trackschanged();
  bitrateManager.capBitrate(5000);
  expect(shakaPlayer.configure).toHaveBeenCalledWith({ abr: { restrictions: { maxBandwidth: 5000000 }, enabled: true }});
  expect(onStreamStateChange).toHaveBeenCalledWith({ maxBitrate: 5000 });
  bitrateManager.capBitrate(1000);
  expect(shakaPlayer.configure).toHaveBeenCalledWith({ abr: { restrictions: { maxBandwidth: 1234567 }, enabled: true }});
...
test('Shaka bitrate manager capBitrate() with Infinity or falsy passed resets max bandwidth and enables ABR.', () => {
  ...
  expect(shakaPlayer.configure).toHaveBeenCalledWith({ abr: { restrictions: { maxBandwidth: Infinity }, enabled: true }});
...
test('Shaka bitrate manager lockBitrate() with "max" passed resets max bandwidth, disables ABR, and selects track with highest bitrate.', () => {
  const { eventHandlers, onStreamStateChange, shakaPlayer, bitrateManager, variantTracks } = setup([1234567, 2345678, 7891234, 3456789]);
  eventHandlers.streaming();
  eventHandlers.trackschanged();
  bitrateManager.lockBitrate('max');
  expect(shakaPlayer.configure).toHaveBeenCalledWith({ abr: { enabled: false, restrictions: { maxBandwidth: Infinity }}});
...
test('Shaka bitrate manager lockBitrate() with no valid number resets max bandwidth, and re-enables ABR.', () => {
  ...
  expect(shakaPlayer.configure).toHaveBeenCalledWith({ abr: { enabled: true, restrictions: { maxBandwidth: Infinity }}});
